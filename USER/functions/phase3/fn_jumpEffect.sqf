...
GRAD_VM_Battleship hideObject true;
...
private _position = GRAD_VM_Battleship modelToWorld [200,400,100];
...
[{
	params ["_lightPoint", "_position"];

	[{
		params ["_args", "_handle"];
		_args params ["_lightPoint", "_position"];

		private _flareSize = GRAD_VM_Battleship getVariable ["flareSize", 0];
		_flareSize = if (!flareShrinking) then {  _flareSize+1000 } else {  _flareSize-1000 };
		GRAD_VM_Battleship setVariable ["flareSize", _flareSize];
		_lightPoint setLightFlareSize _flareSize;
		_lightPoint setLightBrightness _flareSize/10;

		if (_flareSize > 20000) then {
			GRAD_VM_Battleship hideObject false;
			flareShrinking = true;
		...
		if (_flareSize < 100) exitWith { [_handle] call CBA_fnc_removePerFrameHandler; 
	...
	}, 0, [_lightPoint, _position]] call CBA_fnc_addPerFrameHandler;

}, [_lightPoint, _position], 1] call CBA_fnc_waitAndExecute;
